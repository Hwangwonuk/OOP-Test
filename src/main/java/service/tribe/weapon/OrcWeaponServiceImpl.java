...
public class OrcWeaponServiceImpl implements OrcWeaponService {
    @Override
    public void equipWeapon(Orc orc, Blunt blunt) {
        orc.validateAlive(); // 캐릭터가 죽었으면 에러 발생

        /*        validateWeaponCompatibility(human, sword); // 종족에 맞는 무기 장착이 아니라면 에러 발생*/

        orc.setEquipWeapon(true); // 무기 착용여부 true

        calculateWeaponAttackPower(orc, blunt);

        calculateWeaponAttackSpeed(orc, blunt);
    }
    ...
    */
    ...
    private void calculateWeaponAttackPower(Orc orc, Blunt blunt) {
        // 무기로 인해 변경될 공격력 수치
        orc.setWeaponAttackPower(
                orc.getAttackPower()
                        .multiply(blunt.getWeaponStat().get(WeaponStatKeys.ATTACK_POWER_PERCENTAGE))
        );

        // 총 공격력 = 기본 공격력 + 무기로 인해 변경될 수치 + 스킬로 인해 변경된 수치
        orc.setTotalAttackPower(
                orc.getAttackPower()
                        .add(orc.getWeaponAttackPower())
                        .add(orc.getSkillAttackPower())
        );
    }

    private void calculateWeaponAttackSpeed(Orc orc, Blunt blunt) {
        // 무기로 인해 변경될 공격속도 수치
        orc.setWeaponAttackSpeed(
                orc.getAttackSpeed()
                        .multiply(blunt.getWeaponStat().get(WeaponStatKeys.ATTACK_SPEED_PERCENTAGE))
        );

        // 총 공격속도 = 기본 공격속도 + 무기로 인해 변경될 수치 + 스킬로 인해 변경된 수치
        orc.setTotalAttackSpeed(
                orc.getAttackSpeed()
                        .add(orc.getWeaponAttackSpeed())
        );
    }

    @Override
    public void attack(Orc attacker, Monster defender) {
    ...
    private BigDecimal calculateDefenderTotalDamage(Orc attacker, Monster defender) {
        return attacker.getTotalAttackPower().subtract(defender.getDefencePower());
    }

    private void validateAttackReadiness(Orc attacker) {
    ...
    private Boolean isTakeDamageByCounterAttack(Monster attacker, Orc defender) {
        return attacker.getAttackPower().multiply(new BigDecimal("0.7")).compareTo(defender.getTotalDefencePower()) > 0;
    }

    private BigDecimal calculateTribeCounterAttackDamage(Monster attacker, Orc defender) {
